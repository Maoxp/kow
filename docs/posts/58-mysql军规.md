...
>
> - 主键要选择较短的数据类型， `Innodb 引擎`普通索引都会保存主键的值，较短的数据类型可以有效的减少索引的磁盘空间，提高索引的缓存效率.
>
...
>
> 如：``where name!=’shenjian’``，如果存在 name 为 null 值的记录，查询结果就不会包含 name 为 null 值的记录。
...
> 解读：*`SELECT uid FROM t_user WHERE phone=13812345678` 会导致全表扫描，而不能命中 `phone索引`*，猜猜为什么？（这个线上问题不止出现过一次）

（26）**禁止在 WHERE条件的 属性上使用函数或者表达式**
